...
class TestStringMethods(unittest.TestCase):
    def setUp(self):
        self.convert = conversion_methods()
    ...
    def test_octet_list_from_bit_string(self):
        test_value_list = [
        ...
        for (expected_result, bits) in test_value_list:
            self.assertEqual(
                expected_result,
                self.convert.octet_list_from_bit_string(bits)
            )

    def test_parse_octet_to_digit(self):
        test_value_list = [
            (0,'00000000'),
            (255,'11111111'),
            (240,'11110000'),
        ...
        for (expected_result, octet) in test_value_list:
            self.assertEqual(
                expected_result,
                self.convert.parse_octet_to_digit(octet)
            )
    ...
    def test_valid_ipv4(self):
        self.assertTrue(self.convert.ipv4_validation('127.0.0.1'))

    def test_invalid_ipv4(self):
        self.assertFalse(self.convert.ipv4_validation('192.168.1.2.3'))
